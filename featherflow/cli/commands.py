...
def onboard():
    ...
    if config_path.exists():
        ...
        if typer.confirm("Overwrite?"):
            ...
            config = load_config()
            save_config(config)
            agent_name = _normalize_agent_name(getattr(config.agents.defaults, "name", "featherflow"))
...
def gateway(
    ...
    console.print(f"{__logo__} Starting FeatherFlow gateway on port {port}...")
...
def status():
    ...
    console.print(f"{__logo__} FeatherFlow Status\n")
